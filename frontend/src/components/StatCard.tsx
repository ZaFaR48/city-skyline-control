...
export function StatCard({ label, value, delta, tone = "default", icon: Icon, hint }: Props) {
  return (
    <div className="glass rounded-xl p-5 relative overflow-hidden">
      <div className={`absolute -top-12 -right-12 size-40 rounded-full bg-gradient-to-br ${TONE[tone]} blur-2xl opacity-60 pointer-events-none`} />
      <div className="flex items-start justify-between relative">
        ...
        <div className={`size-10 rounded-lg grid place-items-center bg-accent/60 border border-border ${TONE[tone].split(" ").pop()}`}>
